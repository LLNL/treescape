...
class CaliReader(Reader):
    ...
    def __init__(self, path = "", pool_size = 10, inclusive_strings = None ):
        ...
        self.inclusive_strings = ["min#inclusive#sum#time.duration",
    ...
    def __iter__(self):
        self.keys = list(self.xy_idx_by_drill_level.keys())  # Convert keys to a list for indexing
        self.index = 0
        return self
            ...
            if 'path' in rec:
                ...
                for i in range(4):  # 0 to 3
                    if i >= len(rec):  # Avoid index out of range error
                        print(f"rec doesn't have string at index {inclusive_strings[i]}")
                        sys.exit()
                    if inclusive_strings[i] not in rec:  # Check if the string exists and is not empty
                        print(f"rec doesn't have string at index {inclusive_strings[i]}")
                        sys.exit()
    ...
    def combine_and_sort_x_and_y(self, xaxis, nibp ):
        ...
        for path in nibp2:
            ...
            for x, ylist in grouped_data.items():
                ...
                y_min = min(ylist)
                y_max = max(ylist)
                new_ydata.append({"sum": y_sum, "avg": y_avg, "min": y_min, "max": y_max})
