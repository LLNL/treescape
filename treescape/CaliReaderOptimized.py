                ...
                if 'path' in rec:
                    ...
                    for i, metric_key in enumerate(["min", "max", "avg", "sum"]):
                        if inclusive_strings[i] in rec:
                            ...
                            print(f"Warning: {inclusive_strings[i]} not found in {cali_file}")
...
class CaliReaderOptimized(Reader):
    ...
    def __init__(self, path="", pool_size=None, inclusive_strings=None):
        ...
        if pool_size is None:
            pool_size = min(multiprocessing.cpu_count(), 20)  # Cap at 20 to avoid overhead
    ...
    def init(self):
        ...
        with multiprocessing.Pool(self.pool_size) as pool:
            ...
            worker_func = partial(process_cali_file_batch, 
                                inclusive_strings=self.inclusive_strings)
        ...
        print(f"Initialization complete. Processed {len(self.xy_idx_by_drill_level)} unique paths.")
